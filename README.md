...
2. TextEditが開くので、普段通りにつぶやきを入力し、⌘S で保存する(複数行OK)。
3. 表示されているダイアログの「投稿」を押す(`旧来投稿方法/tweets.txt` への追記 +
   HTML再生成 + commit + push まで自動で行われる)。「キャンセル」を押す、または
   何も入力せず投稿すると中止され、`tweets.txt` には何も書き込まれない。
